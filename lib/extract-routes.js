...
const relevantMethods = [ 'GET', 'POST', 'PUT', 'DELETE', 'PATCH' ]
...
function getExpress3Routes (app) {
  ...
  const hash = Object.keys(allRoutes)
    .filter(isRelevantMethod)
    .reduce(function byMethod (acc, k) {
      const routes = allRoutes[k]
...
function getExpress4Routes (app) {
  ...
  const uncondensed = app._router.stack
    .map(getRouteInfo)
    .filter(defined)
